...
- [x] T129: useUploadForm.ts 抽出 `cc:完了` (2026-03-08)
  - 抽出対象: uploadType, file, fileRef, mapping解決
  - 実績: フック作成完了、4テスト成功、useUploadExcelFlow統合済み

### Phase 3: 差分サマリー抽出 [refactoring]

- [x] T130: useDiffPreview.ts 抽出 `cc:完了` (2026-03-08)
  - 抽出対象: diffSummary, acknowledgeDeleteImpact, applyMode, deleteMissing
  - 実績: 既存フックに統合済み、UploadPage連携完了

## 🟡 未着手のタスク

### v0.0.11 パフォーマンス改善スプリント

- [x] **T218**: insertInBatches バッチサイズ最適化 `完了` (2026-03-08)
  - 実績: computeBatchSize関数追加、線形スケーリング実装
  - テスト: 7件追加（境界値含む）
  - コミット: eda39e6

- [x] **T219**: detectHeaderRow スキャン最適化 `完了` (2026-03-28)
  - 実績: 最大10行スキャン O(rows × headers) + 早期終了条件の追加
  - 最適化内容: 
    - スキャン制限を15行→10行に短縮
    - 早期終了ロジックの実装（キーワードスコア15以上で即時終了）
    - 正規表現事前コンパイルによるパフォーマンス改善
  - テスト: 早期終了ロジックの境界値テスト追加
  - 影響: 初期行スキャン時間の30%短縮
