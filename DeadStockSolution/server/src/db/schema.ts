...
export const subscriptionStatusValues = ['trialing', 'active', 'past_due', 'canceled', 'incomplete'] as const;
export type SubscriptionStatus = (typeof subscriptionStatusValues)[number];

export const subscriptionPlans = pgTable('subscription_plans', {
  id: serial('id').primaryKey(),
  name: text('name').$type<SubscriptionPlan>().notNull().unique(),
  displayName: varchar('display_name', { length: 64 }).notNull(),
  monthlyPriceYen: integer('monthly_price_yen').notNull(),
  maxPharmacies: integer('max_pharmacies').notNull(),
  ...
  chkSubscriptionPlansName: check('chk_subscription_plans_name', sql`${table.name} IN ('light', 'standard', 'enterprise')`),
}));

export const pharmacySubscriptions = pgTable('pharmacy_subscriptions', {
  id: serial('id').primaryKey(),
  pharmacyId: integer('pharmacy_id').notNull().references(() => pharmacies.id, { onDelete: 'cascade' }).unique(),
  planId: integer('plan_id').notNull().references(() => subscriptionPlans.id),
  status: text('status').$type<SubscriptionStatus>().notNull().default('trialing'),
  stripeCustomerId: varchar('stripe_customer_id', { length: 255 }),
  stripeSubscriptionId: varchar('stripe_subscription_id', { length: 255 }),
  trialStartsAt: timestamp('trial_starts_at', { mode: 'string' }),
  trialEndsAt: timestamp('trial_ends_at', { mode: 'string' }),
  currentPeriodStartsAt: timestamp('current_period_starts_at', { mode: 'string' }),
  currentPeriodEndsAt: timestamp('current_period_ends_at', { mode: 'string' }),
  canceledAt: timestamp('canceled_at', { mode: 'string' }),
  cancelAtPeriodEnd: boolean('cancel_at_period_end').default(false).notNull(),
  createdAt: timestamp('created_at', { mode: 'string' }).defaultNow(),
  updatedAt: timestamp('updated_at', { mode: 'string' }).defaultNow(),
}, (table) => ({
  idxPharmacySubscriptionsPharmacy: index('idx_pharmacy_subscriptions_pharmacy').on(table.pharmacyId),
  idxPharmacySubscriptionsStatus: index('idx_pharmacy_subscriptions_status').on(table.status),
  idxPharmacySubscriptionsStripeCustomer: index('idx_pharmacy_subscriptions_stripe_customer').on(table.stripeCustomerId),
  idxPharmacySubscriptionsStripeSubscription: index('idx_pharmacy_subscriptions_stripe_subscription').on(table.stripeSubscriptionId),
  chkPharmacySubscriptionsStatus: check('chk_pharmacy_subscriptions_status', sql`${table.status} IN ('trialing', 'active', 'past_due', 'canceled', 'incomplete')`),
}));

export const subscriptionInvoices = pgTable('subscription_invoices', {
  id: serial('id').primaryKey(),
  subscriptionId: integer('subscription_id').notNull().references(() => pharmacySubscriptions.id, { onDelete: 'cascade' }),
  ...
  idxSubscriptionInvoicesSubscription: index('idx_subscription_invoices_subscription').on(table.subscriptionId),
  idxSubscriptionInvoicesStatus: index('idx_subscription_invoices_status').on(table.status),
  idxSubscriptionInvoicesStripeInvoice: index('idx_subscription_invoices_stripe_invoice').on(table.stripeInvoiceId),
}));
...
export const subscriptionEvents = pgTable('subscription_events', {
  ...
  idxSubscriptionEventsSubscription: index('idx_subscription_events_subscription').on(table.subscriptionId),
  idxSubscriptionEventsType: index('idx_subscription_events_type').on(table.eventType),
  idxSubscriptionEventsStripeEvent: index('idx_subscription_events_stripe_event').on(table.stripeEventId),
}));
