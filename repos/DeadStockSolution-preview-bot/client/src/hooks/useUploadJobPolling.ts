...
// === 型定義 ===
export type UploadProgressPhase = 'idle' | 'previewing' | 'queueing' | 'pending' | 'processing' | 'completed' | 'failed';
...
export function isTransientUploadJobPollingError(err: unknown): boolean {
  ...
  if (err instanceof Error) {
    ...
    if (message.includes('network') || message.includes('timeout') || message.includes('タイムアウト') || message.includes('ネットワーク')) {
      return true;
    }
  }
  return false;
