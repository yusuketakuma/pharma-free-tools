...
describe('UploadPage', () => {
  ...
  it('shows excel and camera flows side-by-side while keeping excel validation visible', async () => {
    ...
    // カメラパネルはSuspenseで遅延読み込みされるためwaitForで待機
    await waitFor(() => {
  ...
  it(
    'retries polling when a transient job status error occurs',
    async () => {
      let jobStatusCallCount = 0;
      const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
        const url = typeof input === 'string' ? input : input.toString();
  ...
  it(
    'keeps job id message when polling keeps failing after retry limit',
    async () => {
      let jobStatusCallCount = 0;
      const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
        const url = typeof input === 'string' ? input : input.toString();
  ...
  it(
    'shows failed async job error and clears queue message',
    async () => {
      const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
        const url = typeof input === 'string' ? input : input.toString();
        if (url.includes('/api/auth/me')) {
