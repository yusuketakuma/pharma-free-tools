...
exports.DRUG_CATEGORIES = void 0;
exports.extractLatestIndexUrl = extractLatestIndexUrl;
exports.extractExcelUrls = extractExcelUrls;
exports.discoverMhlwExcelUrls = discoverMhlwExcelUrls;
...
/** MHLW 薬価基準の4カテゴリ */
exports.DRUG_CATEGORIES = ['内用薬', '外用薬', '注射薬', '歯科用薬剤'];
function validateMhlwHost(url) {
    try {
...
function inferCategoryFromFilename(url) {
    const filename = url.split('/').pop() || '';
    // MHLW 命名規則: _01=内用薬, _02=外用薬, _03=注射薬, _04=歯科用薬剤
    if (/_01[\._]/.test(filename) || filename.endsWith('_01.xlsx') || filename.endsWith('_01.xls'))
        return '内用薬';
    if (/_02[\._]/.test(filename) || filename.endsWith('_02.xlsx') || filename.endsWith('_02.xls'))
        return '外用薬';
    if (/_03[\._]/.test(filename) || filename.endsWith('_03.xlsx') || filename.endsWith('_03.xls'))
        return '注射薬';
    if (/_04[\._]/.test(filename) || filename.endsWith('_04.xlsx') || filename.endsWith('_04.xls'))
        return '歯科用薬剤';
    return null;
}
async function fetchHtml(url, dispatcher) {
 ...
 */
function extractLatestIndexUrl(html, baseUrl) {
 ...
 */
function extractExcelUrls(html, baseUrl) {
    const results = [];
    ...
    while ((match = linkPattern.exec(html)) !== null) {
        ...
        if (!resolved || !validateMhlwHost(resolved))
            continue;
        const category = inferCategoryFromLabel(linkText)
 ...
 */
async function discoverMhlwExcelUrls(portalUrl = MHLW_PORTAL_URL) {
    if (!validateMhlwHost(portalUrl)) {
        throw new Error(`ポータル URL のホスト名が *.mhlw.go.jp ではありません: ${portalUrl}`);
    }
    const validated = await (0, network_utils_1.validateExternalHttpsUrl)(portalUrl);
    if (!validated.ok) {
        throw new Error(`ポータル URL の検証に失敗: ${validated.reason}`);
    }
    const pinnedAgent = (0, network_utils_1.createPinnedDnsAgent)(validated.hostname ?? new URL(portalUrl).hostname, validated.resolvedAddresses);
    const dispatcher = pinnedAgent;
    try {
        // Step 1: 親ポータルを取得
        logger_1.logger.info('MHLW index scraper: fetching portal page', { url: portalUrl });
        ...
        logger_1.logger.info('MHLW index scraper: found index page', { indexUrl });
        // Step 3: インデックスページを取得（HTTPS + DNS pinning を再検証、同一ホストならエージェント再利用）
        const indexValidated = await (0, network_utils_1.validateExternalHttpsUrl)(indexUrl);
        ...
        const indexPinnedAgent = sameHost
            ? null
            : (0, network_utils_1.createPinnedDnsAgent)(indexHostname, indexValidated.resolvedAddresses);
        const indexDispatcher = sameHost ? dispatcher : indexPinnedAgent;
        try {
            ...
            logger_1.logger.info('MHLW index scraper: discovered files', {
                indexUrl,
            ...
            if (indexPinnedAgent)
                await indexPinnedAgent.close().catch(() => undefined);
        }
    }
    finally {
...
//# sourceMappingURL=mhlw-index-scraper.js.map