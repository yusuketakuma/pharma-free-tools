...
export function getExpiryDateRange(): { todayStr: string; threeDaysLaterStr: string } {
  ...
  // ローカルタイムゾーン（JST）で日付を計算し、expirationDateIso と一貫性を保つ
  ...
  return {
    ...
    threeDaysLaterStr: formatDateIso(threeDaysLater),
  };
}
...
export async function fetchAdminMessageEvents(
  ...
  // 重複排除してマージ
  let merged = dedupeRowsById<AdminMessageRow>([...allMessages, ...pharmacyMessages]);

  // 全体で時系列降順に再ソート（マージ前は各クエリ内でのみソート済み）
  merged.sort((a, b) => {
