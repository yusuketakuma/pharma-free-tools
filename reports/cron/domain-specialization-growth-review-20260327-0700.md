# Domain Specialization Growth Review — 2026-03-27 07:00 JST

## 結論
- いまやるべきは、専属エージェントを増やすこと自体ではなく、**領域ごとの判断パックを固定して再利用率を上げること**。
- 優先順は前回と同じでよいが、今回は「知識の置き場」と「検証の型」を先に固めるべき段階。
- 推奨順:
  1. **dss-manager**: 接続・停止・後片付けを含む運用安全化を標準化
  2. **pharma-free-tools**: 既存HTMLの低リスク改善と公開前監査を専任化
  3. **sidebiz**: 候補列挙ではなく PoC 接続を専任化
  4. **polymarket**: 収益化ではなく compliance / paper trading / stop rule を先に固定
- 追加エージェントは、境界が明確で反復頻度が高いものだけに限定するのが最も安全。

## 特化候補領域

### 1) dss-manager / DDS-agents
**確認できた事実**
- 既存レビューでは、register / heartbeat / claim / callback / report の E2E は通っている。
- ただし、`bootstrap token` や `state.json` の扱い、`status=idle` なのに stale な `active*` が残る問題など、**運用後の後片付け** が課題として残っている。
...
- `reportUrl null` などの例外確認

**repo/domain 専用に持つべきもの**
- verification commands
  - runner scripts の `node --check`
  - live register / heartbeat / claim の接続確認
  - callback / report 到達確認
  - cleanup 確認
- 既知障害
  - `.env.local` に bootstrap token が残る
  - `state.json` に active state が残る
  - `status=idle` なのに stale `active*` が残る
  - `reportUrl` が null になることがある
- 既知修正パターン
  - register 成功時に token を自動除去
  - completed / failed / blocked 後に active state を clear
  - live 接続後に secret / state を即片付け
- よく使う比較軸
  - 接続済みか
  - 安全に止まるか
  - 再実行できるか
...
### 2) pharma-free-tools
**確認できた事実**
- `projects/pharma-free-tools/CLAUDE.md` と `project.yaml` で、日次の新規作成 / 既存改善 / push の流れが定義されている。
- 領域の中心は、調査よりも **既存HTMLの低リスク改善と公開前品質監査** にある。
...
- verification commands
  - daily tool 実行
  - `--skip-new` / `--skip-update` の分岐確認
  - GitHub Actions / run list 確認
  - `html` 実ファイル数確認
...
### 3) sidebiz
**確認できた事実**
- `docs/sidebiz/scout-rubric.md` で、1アイデアごとの必須項目と比較軸が定義されている。
- 直近の scout では、候補列挙より **PoC 接続** が重要になっている。
...
### 4) polymarket
**確認できた事実**
- 技術的には BOT 構築可能だが、実務の難所は監視・在庫管理・heartbeat・報酬条件・解決遅延・規制チェックにある。
- 本番売買より前に、**監視基盤 + compliance gate + paper trading** を固める方が安全。
...
- `blocked / question / pr / report` を JSON-only で分岐する実務を固定する
- いま必要なのは新規機能より **E2E 1件 + cleanup 標準化**
...
- 似た痛点でも入口が違えば別候補
- 需要確認だけで終わると改善が積み上がらない

### polymarket
- 収益性評価より前に **compliance / risk / stop rule** が必要
- 技術調査が十分でも、運用可能性が未形成なら進めない
...
1. **pharma-free-tools-auditor**
   ...
   - 理由: 調査→監査→低リスク修正が反復しており、専任化の効果が高い
